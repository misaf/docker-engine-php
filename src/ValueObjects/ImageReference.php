...
final class ImageReference implements Stringable
{
    ...
    {
        if ('' === mb_trim($value) || mb_trim($value) !== $value || preg_match('/\s/', $value)) {
        ...
        if (str_contains($value, '@')) {
            ...
            if (false === $separator) {
                throw new InvalidArgumentException('The image reference contains an invalid digest.');
            }

            $this->repository = mb_substr($value, 0, $separator);
            $this->tag = '';
            $this->digest = mb_substr($value, $separator + 1);

            if (1 !== mb_substr_count($value, '@')
                || '' === $this->repository
                || 1 !== preg_match('/^[A-Za-z][A-Za-z0-9._+-]*:[A-Za-z0-9=_-]+$/', $this->digest)) {
                throw new InvalidArgumentException('The image reference contains an invalid digest.');
            }
        ...
        if ('' === $this->repository
            || 1 !== preg_match('/^[A-Za-z0-9_][A-Za-z0-9_.-]{0,127}$/', $this->tag)) {
            throw new InvalidArgumentException('The image reference contains an invalid tag.');
        }
