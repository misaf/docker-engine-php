...
abstract class GeneratedApi
{
    ...
    protected function call(Endpoint $endpoint, ?GeneratedRequest $input = null): object|string|null
    {
        ...
        if (in_array($endpoint->responseKind, [ResponseKind::Stream, ResponseKind::Progress], true)) {
            $response = $this->transport->stream($request);
            $this->assertStreamSuccess($response);

            return ResponseKind::Progress === $endpoint->responseKind ? new ProgressStream($response->stream) : $response;
        }
        ...
        if (ResponseKind::Raw === $endpoint->responseKind) {
        ...
        return $this->serializer->denormalize(
            ResponseKind::JsonArray === $endpoint->responseKind ? ['items' => $data] : $data,
    ...
    private function request(Endpoint $endpoint, ?GeneratedRequest $input): Request
    {
        ...
        if (null !== $endpoint->upgrade) {
            $headers += ['Connection' => 'Upgrade', 'Upgrade' => $endpoint->upgrade->value];

            if (ConnectionUpgrade::WebSocket === $endpoint->upgrade) {
                $headers += [
